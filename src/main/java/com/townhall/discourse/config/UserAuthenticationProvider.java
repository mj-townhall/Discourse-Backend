...
public class UserAuthenticationProvider {
    ...
    @PostConstruct
    protected void init() {
        // Encode the secret key to Base64 to avoid having the raw secret key available in the JVM
        secretKey = Base64.getEncoder().encodeToString(secretKey.getBytes());
    }

    public String createToken(UserDto user) {
        Date now = new Date();
        Date validity = new Date(now.getTime() + 3600000); // 1 hour session time in milliseconds
